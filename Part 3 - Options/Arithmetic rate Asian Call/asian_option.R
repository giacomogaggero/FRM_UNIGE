...
# Price simulation at dates
S_t1 <- S * exp(((r - q) - 0.5 * sigma^2) * tau[1] + sigma * eps1 * sqrt(tau[1]))
S_t2 <- S_t1 * exp(((r - q) - 0.5 * sigma^2) * (tau[2] - tau[1]) + sigma * eps2 * sqrt(tau[2] - tau[1]))
S_t3 <- S_t2 * exp(((r - q) - 0.5 * sigma^2) * (tau[3] - tau[2]) + sigma * eps3 * sqrt(tau[3] - tau[2]))

# arithmetic mean and payoff
